...
SERVICE_NAME = "ai.memova.mcp-connection-helper"
...
class MacOSCredentialStore:
    ...
    def get(self, account: str) -> str | None:
        ...
        try:
            ...
            if status != 0:
                raise RuntimeError(
                    "Unable to read the Memova MCP credential from macOS Keychain."
                )
    ...
    def set(self, account: str, secret: str) -> None:
        ...
        try:
            ...
            if status != 0:
                raise RuntimeError(
                    "Unable to store the Memova MCP credential in macOS Keychain."
                )
        finally:
            self._release(item)
            if release_keychain:
    ...
    def delete(self, account: str) -> None:
        ...
        try:
            ...
            if status != 0 or security.SecKeychainItemDelete(item) != 0:
                raise RuntimeError(
                    "Unable to delete the Memova MCP credential from macOS Keychain."
                )
        finally:
            self._release(item)
            if release_keychain:
...
class LinuxCredentialStore:
    ...
    def set(self, account: str, secret: str) -> None:
        ...
        if completed.returncode != 0:
            raise RuntimeError("Unable to store the Memova MCP credential in Secret Service.")
    ...
    def delete(self, account: str) -> None:
        ...
        if completed.returncode not in {0, 1}:
            raise RuntimeError("Unable to delete the Memova MCP credential from Secret Service.")
...
class WindowsCredentialStore:
    ...
    def get(self, account: str) -> str | None:
        ...
        if not result:
            if ctypes.windll.kernel32.GetLastError() == self.ERROR_NOT_FOUND:
                return None
            raise RuntimeError(
                "Unable to read the Memova MCP credential from Credential Manager."
            )
        try:
    ...
    def set(self, account: str, secret: str) -> None:
        ...
        if not ctypes.windll.advapi32.CredWriteW(ctypes.byref(credential), 0):
            raise RuntimeError(
                "Unable to store the Memova MCP credential in Credential Manager."
            )
    ...
    def delete(self, account: str) -> None:
        ...
        if not result and ctypes.windll.kernel32.GetLastError() != self.ERROR_NOT_FOUND:
            raise RuntimeError(
                "Unable to delete the Memova MCP credential from Credential Manager."
            )
...
def decode_secret(value: str) -> dict:
    ...
    if not isinstance(decoded, dict):
        raise TypeError("Stored Memova MCP credential has an invalid format.")
    return decoded
